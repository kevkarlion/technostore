...
export function extractTokens(normalizedQuery: string): TokenizedQuery {
  ...
  // Priority 1: Capacity — test merged pairs first, then individual tokens
  ...
  // Priority 5: Model — remaining alphanumeric tokens
  for (let i = 0; i < words.length; i++) {
    if (consumed.has(i)) continue;
    if (/^[a-z0-9][\w\-]*$/i.test(words[i]) && words[i].length >= 2) {
      model = words[i];
      consumed.add(i);
      break;
    }
  ...
  const expandedRaw = expand(words);
